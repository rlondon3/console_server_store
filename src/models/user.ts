...
export class UserStore {
    ...
    async create(user: User): Promise<User> {
        try {
            const conn: PoolClient = await client.connect();
            const sql =
            'INSERT INTO users (firstname, lastname, email, address, username, password, isAdmin) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *';
    ...
    async update(user: User, _id?: number): Promise<User> {
        try {
            const sql = 
            'UPDATE users SET firstname=($1), lastname=($2), email=($3), address=($4), username=($5), password=($6), isAdmin=($7) WHERE id=($8) RETURNING *';
            const conn: PoolClient = await client.connect();
            const hash = bcrypt.hashSync(
